...
def perform(filename):
	...
	z_list = [getz(l) for l in lines if getz(l)]
	layer_height = round(z_list[-1] - z_list[-2], 1)
	height = round(z_list[-1], 1)
	...
	temp_steps = abs(int(input(  "Enter the amount at which temperature should change : ")))
	base_height=     int(input(  "Enter the height of the base in mm (0 if none)      : "))

	if final_temp < start_temp:
		final_temp -= 2
		temp_steps = -temp_steps
	temps = list(range(start_temp, final_temp+1, temp_steps))
	temp_change_height = round((height-base_height)/len(temps), 1)
	...
	if base_height > 0:
		print("%5.1f to %5.1f mm - %d degrees  (base)" % (0, base_height, temps[0]))
	for i in range(0, len(temps)):
		print("%5.1f to %5.1f mm - %d degrees" % \
		((i*temp_change_height)+base_height, height if i==len(temps)-1 else ((i+1)*temp_change_height)+base_height, temps[i]))
	...
	if not inserted_M109 and not inserted_M104:
		...
		sys.exit()
	if len(extra_M104) > 1:
		print("\nWARNING: There are more than one M104 commands already present in the gcode...")
	...
	for i in range(1, len(temps)):
		z = (i*temp_change_height)+base_height
